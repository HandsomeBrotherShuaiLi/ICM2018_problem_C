...
def ml_model(data,state,feature,y):
    ...
    model.fit(Xcv_train.as_matrix(),ycv_train.as_matrix(),epochs=10000,batch_size=16)
    t=model.predict(Xcv_test.as_matrix())
    plt.figure('prediction  '+state+'  '+y[0])
    plt.plot(range(len(t)), t, 'b', label=state+'  '+y[0]+"   predict")
    plt.plot(range(len(t)), ycv_test, 'r', label=state+'  '+y[0]+"   test")
    plt.legend(loc="upper right")  # 显示图中的标签
    plt.xlabel("the number of "+y[0])
    plt.ylabel('value of '+y[0])
    plt.savefig('cross validation  ' + state + '  ' + y[0] + '.png')
    ...
    for i in range(len(t)):
        ...
        MSE+=((h[i]-ycv_test.values[i])/(ycv_test.values[i]))**2
    MSE=1.0*MSE/33
    #
    ...
    # print('MSE',MSE)
    yuce.loc['cross validation MSE'] =[None,MSE]
    # plt.legend(loc="upper right")  # 显示图中的标签
